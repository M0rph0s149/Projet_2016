	...
	* @return true si les segments se touchent et false sinon.  
    ...
    public static boolean segmentsCroises(Coordonnees p1, Coordonnees p2,
        ...
        if (p1.equals(p2)) {
            if (q1.equals(q2)) {
                if (p1.equals(q1)) {
                    return true; // 2 segments dégénérés et identiques
                }
            ...
            if ((q1Lat - q2Lat) * (p1Long - q1Long) == (q1Long - q2Long) * (p1Lat - q1Lat)) {
                if (p1Lat >= Math.min(q1Lat, q2Lat) && p1Lat <= Math.max(q1Lat, q2Lat)) {
                    return true;
                }
            }
            return false;
        ...
        if (q1.equals(q2)) {
            ...
            if ((p1Lat - p2Lat) * (q1Long - p1Long) == (p1Long - p2Long) * (q1Lat - p1Lat)) {
                if (q1Lat >= Math.min(p1Lat, p2Lat) && q1Lat <= Math.max(p1Lat, p2Lat)) {
                    return true;
                }
            }
            return false;
        ...
        if (p1Lat == p2Lat && q1Lat == q2Lat) {
            ...
            if (Math.min(q1Long, q2Long) >= Math.min(p1Long, p2Long)
                    && Math.min(q1Long, q2Long) <= Math.max(p1Long, p2Long)) {
                return true;
            }
            if (Math.min(p1Long, p2Long) >= Math.min(q1Long, q2Long)
                    && Math.min(p1Long, p2Long) <= Math.max(q1Long, q2Long)) {
                return true;
            }
            return false;
        }
        ...
        if (p1Long == p2Long && q1Long == q2Long) {
            ...
            if (Math.min(q1Lat, q2Lat) >= Math.min(p1Lat, p2Lat)
                    && Math.min(q1Lat, q2Lat) <= Math.max(p1Lat, p2Lat)) {
                return true;
            }
        ...
        cLong = ((double) ((q1Long - q2Long) * (p1Lat * p2Long - p1Long * p2Lat)
                ...
                / ((p1Lat - p2Lat) * (q1Long - q2Long) - (q1Lat - q2Lat) * (p1Long - p2Long));
        if (cLat < Math.min(p1Lat, p2Lat) || cLat > Math.max(p1Lat, p2Lat)) {
            return false;
        }
        if (cLat < Math.min(q1Lat, q2Lat) || cLat > Math.max(q1Lat, q2Lat)) {
            return false;
        }
        if (cLong < Math.min(p1Long, p2Long) || cLong > Math.max(p1Long, p2Long)) {
            return false;
        }
        if (cLong < Math.min(q1Long, q2Long) || cLong > Math.max(q1Long, q2Long)) {
            return false;
        }
        return true;
    }
